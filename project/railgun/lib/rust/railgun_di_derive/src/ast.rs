...
impl<'node> Input<'node> {
    pub fn from_syn(node: &'node DeriveInput) -> Result<Self> {
        match &node.data {
            Data::Struct(data) => Ok(Input::Struct(Struct::from_syn(node, data)?)),
            Data::Enum(data) => Ok(Input::Enum(Enum::from_syn(node, data)?)),
            Data::Union(_) => Err(Error::new_spanned(node, "unions cannot be used as errors")),
        }
    }
}
...
impl<'node> Field<'node> {
    fn multiple_from_syn(fields: &'node Fields) -> Result<Vec<Self>> {
        fields
            .iter()
            .enumerate()
    ...
    fn from_syn(index: usize, field: &'node syn::Field) -> Result<Self> {
        Ok(Field {
            original: field,
            attributes: FieldAttributes::from_attributes(&field.attrs)?,
            member: match &field.ident {
                Some(ident) => Member::Named(ident.clone()),
                None => Member::Unnamed(Index {
                    index: u32::try_from(index)
                        .map_err(|_| Error::new_spanned(field, "index out of bounds"))?,
                    span: field.span(),
                }),
            },
            ty: &field.ty,
