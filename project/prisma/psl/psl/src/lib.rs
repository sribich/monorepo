...
pub trait SchemaExt {
    ///
    fn parse_configuration(self) -> Result<Configuration, Diagnostics>;
}

impl SchemaExt for Schema {
    fn parse_configuration(self) -> Result<Configuration, Diagnostics> {
        let schema = self.parse().into_configured();

        schema
            .diagnostics()
            .take()
            .to_result()
            .map(|_| schema.configuration().clone())
    }
}
...
pub fn parse_configuration(schema: &str) -> Result<Configuration, Diagnostics> {
    Schema::from_content(schema).parse_configuration()
}
...
pub fn parse_configuration_multi_file(
    ...
    psl_core::parse_configuration_multi_file(files)
}
...
pub fn validate(file: SourceFile, extension_types: &dyn ExtensionTypes) -> ValidatedSchema {
    psl_core::validate(file, extension_types)
}
