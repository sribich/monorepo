...
use psl::{Configuration, SchemaExt, SourceFile, parse_configuration_multi_file};
...
pub(crate) fn parse_config(schema: &str) -> Result<Configuration, String> {
    Schema::from_content(schema)
        .parse_configuration()
        .map_err(|err| err.to_pretty_string("schema.prisma", schema))
}

#[track_caller]
pub(crate) fn parse_configuration(datamodel_string: &str) -> Configuration {
    match Schema::from_content(datamodel_string).parse_configuration() {
        Ok(c) => c,
        Err(errs) => {
            panic!(
