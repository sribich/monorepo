...
pub fn parse(
    ...
    match (filter_key, value) {
        ...
        (filters::NONE, Some(value)) => {
            Ok(field.no_related(extract_filter(value, field.related_model())?))
        }
        (filters::EVERY, Some(value)) => {
            Ok(field.every_related(extract_filter(value, field.related_model())?))
        }

        // One-relation filters
        (filters::IS, Some(value)) => {
            Ok(field.to_one_related(extract_filter(value, field.related_model())?))
        }
