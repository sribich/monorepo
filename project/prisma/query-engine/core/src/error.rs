...
impl CoreError {
    ...
    pub(crate) fn is_transient(&self) -> bool {
        match self {
            CoreError::InterpreterError(InterpreterError::ConnectorError(err)) => {
                err.is_transient()
            }
...
impl From<CoreError> for user_facing_errors::Error {
    fn from(err: CoreError) -> user_facing_errors::Error {
        match err {
            CoreError::TransactionError(err) => user_facing_errors::KnownError::new(
            ...
            CoreError::QueryGraphBuilderError(
                ...
                user_facing_errors::query_engine::MissingRequiredArgument {
                    argument_name,
                    field_name,
                    object_name,
                },
            ...
            CoreError::QueryGraphBuilderError(QueryGraphBuilderError::RelationViolation(
                RelationViolation {
                    relation,
            ...
            | CoreError::InterpreterError(InterpreterError::QueryGraphBuilderError(
                ...
                user_facing_errors::query_engine::RelationViolation {
            ...
            | CoreError::InterpreterError(InterpreterError::QueryGraphBuilderError(
                ...
                user_facing_errors::query_engine::ConnectedRecordsNotFound { details },
            )
            .into(),

            CoreError::QueryGraphBuilderError(QueryGraphBuilderError::InputError(details)) => {
                user_facing_errors::KnownError::new(user_facing_errors::query_engine::InputError {
                    details,
                })
                .into()
            }

            CoreError::InterpreterError(InterpreterError::InterpretationError(
                msg,
                Some(cause),
            )) => match cause.as_ref() {
                InterpreterError::QueryGraphBuilderError(
                    QueryGraphBuilderError::RecordNotFound(cause),
                ) => user_facing_errors::KnownError::new(
                ...
                InterpreterError::QueryGraphBuilderError(
                    ...
                    user_facing_errors::query_engine::RecordsNotConnected {
                ...
                _ => user_facing_errors::KnownError::new(
                    user_facing_errors::query_engine::InterpretationError {
            ...
            CoreError::FieldConversionError(FieldConversionError {
                ...
                user_facing_errors::query_engine::MissingFieldsInModel {
                    field,
                    expected_type,
                    found,
                },
            ...
            CoreError::ConversionError { .. } => user_facing_errors::KnownError::new(
                user_facing_errors::query_engine::ValueOutOfRange {
