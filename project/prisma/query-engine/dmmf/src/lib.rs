...
impl ValidatedSchemaDmmfExt for Schema<Validated> {
    fn into_dmmf(self) -> DataModelMetaFormat {
        let schema = self.into_context();

        from_precomputed_parts(&schema::build(Arc::new(schema), true))
    }
}

pub fn dmmf_from_validated_schema(schema: Arc<ValidatedSchema>) -> DataModelMetaFormat {
