...
fn nested_upsert_list_input_object(
    ...
    let create_types =
        create_one::create_one_input_types(ctx, related_model.clone(), Some(parent_field.clone()));
    let update_types =
        update_one_objects::update_one_input_types(ctx, related_model, Some(parent_field.clone()));

    let ident = Identifier::new_prisma(IdentifierType::NestedUpsertManyInput(
...
fn nested_upsert_nonlist_input_object(
    ...
    let ident = Identifier::new_prisma(IdentifierType::NestedUpsertOneInput(
        parent_field.related_field(),
    ));
    ...
    input_object.set_fields(move || {
        let update_types = update_one_objects::update_one_input_types(
            ctx,
            related_model.clone(),
