...
impl QueryParamsWhere {
    ...
    where
        ...
        V: Into<Vec<F>>,
    {
        QueryParamsWhere::CompoundIdentifier(
            fields.into().into_iter().map(|f| f.into()).collect(),
            arg_name.into(),
        )
    }
...
impl QueryParamsWhereMany {
    ...
    where
        ...
        V: Into<Vec<F>>,
    {
        QueryParamsWhereMany::ManyCompounds(
            fields.into().into_iter().map(|f| f.into()).collect(),
            arg_name.into(),
        )
    }
