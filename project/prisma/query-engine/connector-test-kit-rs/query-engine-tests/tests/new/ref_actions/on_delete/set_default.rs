...
#[test_suite(suite = "setdefault_onD_1to1_req", exclude(MySQL))]
mod one2one_req {
...
#[test_suite(suite = "setdefault_onD_1to1_opt", exclude(MySQL))]
mod one2one_opt {
    ...
    /// Deleting the parent reconnects the child to the default and fails (the default doesn't exist).
    #[connector_test(schema(optional_with_default), exclude(MySQL))]
    async fn delete_parent_no_exist_fail(runner: Runner) -> TestResult<()> {
        insta::assert_snapshot!(
          run_query!(&runner, r#"mutation { createOneParent(data: { id: 1, child: { create: { id: 1 }}}) { id }}"#),
          @r###"{"data":{"createOneParent":{"id":1}}}"###
...
#[test_suite(suite = "setdefault_onD_1toM_req", exclude(MySQL))]
mod one2many_req {
    fn required_with_default() -> String {
        let schema = indoc! {
            r#"model Parent {
...
#[test_suite(suite = "setdefault_onD_1toM_opt", exclude(MySQL))]
mod one2many_opt {
    fn optional_with_default() -> String {
        let schema = indoc! {
            r#"model Parent {
