...
impl ExistingPath {
    pub fn new<S: AsRef<str>>(path: S) -> Result<Self, ExistingPathError> {
    ...
    pub fn validate(&self) -> Result<(), ExistingPathError> {
...
mod test {
    ...
    fn ok_on_existing_path() {
        let path = ExistingPath::new("./Cargo.toml");
    ...
    fn err_on_nonexistent_path() {
        let path = ExistingPath::new("./THIS_FILE_DOES_NOT_EXIST");
