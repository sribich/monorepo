...
test("lastJsonMessage updates with a json object when websocket receives a message", async () => {
    const { result } = renderHook(() => useWebSocket<{ name: string }>(URL, options))
    await server.connected
...
test("shared websockets receive updates as if unshared", async () => {
    const { result: component1 } = renderHook(() => useWebSocket(URL, options))
    await server.connected

    const { result: component2 } = renderHook(() => useWebSocket(URL, options))
    await server.connected

    const { result: component3 } = renderHook(() => useWebSocket(URL, options))
    await server.connected
...
test("Options#eventSourceOptions, if provided, instantiates an EventSource instead of a WebSocket", async () => {
    ...
    const { result, rerender } = renderHook(() => useWebSocket(URL, options))
    rerender()
...
test.each([
    false,
    true,
])("Options#heartbeat, if provided, sends a message to the server at the specified interval and works when share is %s", async (shareOption) => {
    options.heartbeat = {
        message: "ping",
        timeout: 10000,
        interval: 500,
...
test.each([
    false,
    true,
])("Options#heartbeat, if provided, close websocket if no message is received from server within specified timeout and works when share is %s", async (shareOption) => {
    options.heartbeat = {
        message: "ping",
        timeout: 1000,
        interval: 500,
...
test.each([
    false,
    true,
])("Options#heartbeat, if provided, do not close websocket if a message is received from server within specified timeout and works when share is %s", async (shareOption) => {
    options.heartbeat = {
        message: "ping",
        timeout: 1000,
        interval: 500,
...
test.each([
    false,
    true,
])("Options#heartbeat, if provided, lastMessage is updated if server message does not matches the returnMessage property of heartbeatOptions and works when share is %s", async (shareOption) => {
    options.heartbeat = {
        message: "ping",
        returnMessage: "pong",
        timeout: 1000,
