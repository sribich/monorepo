...
export const DateFilterContent = (props: PropertyFilterProps) => {
    return null
}

export const DateFilter = (_props: PropertyFilterProps) => {
    ...
    const propertyName = schema.property.find(props.filter.property)?.name
        ...
        <DialogTrigger>
            <DelegateButton>
                <Chip size="md" className="flex items-center">
                    {createElement(DateProperty.icon, { size: 16, className: "mr-1" })}
                    <span>{propertyName}</span>:<span>{content}</span>
                    <ChevronDown size="16" />
                </Chip>
            </DelegateButton>
            <Popover>
                    ...
                    <Box padding="2" rounded="md">
                        <Flex className="mb-2">
                            <Select
                                ...
                                size="xs"
                                variant="light"
                                defaultSelectedKey={filter.kind}
                                items={filterKeys}
                            >
                                {(item) => <SelectItem id={item.key}>{item.key}</SelectItem>}
                            </Select>
                            <DialogTrigger>
