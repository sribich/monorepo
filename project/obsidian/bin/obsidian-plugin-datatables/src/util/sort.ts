// @ts-nocheck
// https://github.com/snovakovic/fast-sort#benchmark

// >>> INTERFACES <<<

type IOrder = 1 | -1

export type IComparer = (a: any, b: any, order: IOrder) => number
...
export function createNewSortInstance(
    opts: ISortInstanceOptions,
): <T>(arrayToSort: T[]) => IFastSort<T>
export function createNewSortInstance(
    opts: ISortInstanceOptions,
): <T>(arrayToSort: T[]) => IFastSort<T> {
    ...
    return function <T>(arrayToSort: T[]): IFastSort<T> {
        const ctx =
            Array.isArray(arrayToSort) && !opts.inPlaceSorting ? arrayToSort.slice() : arrayToSort
