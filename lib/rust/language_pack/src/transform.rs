...
pub struct JapaneseTransforms {}

// static JAPANESE_TRANSFORMS = OnceCell::new();

pub struct Transform {
    name: &'static str,
    description: &'static str,
    rules: &'static [Inflection],
}
...
impl TextTransform {
    ...
    pub fn resolve(
    ...
    ) -> Vec<(Option<TextIdk>, String)> {
        let mut work = &self.text[..];
        let mut matches = vec![];
        ...
        while !work.is_empty() {
                ...
                .filter_map(|it| {
                    ...
                    for i in 0..24 {
                        if let Some((index, _)) = it.text.char_indices().nth_back(i) {
                            // TODO: We can just make a CStr here to test. No alloc
                            if adaptive.contains_key(&CString::new(&it.text[index..]).unwrap()) {
            ...
            if work == &self.text[0..(result[0].0.text.len() - result[0].1.len())] {
                break;
            }

            work = &self.text[0..(result[0].0.text.len() - result[0].1.len())];
    ...
    fn resolve_from<'a>(&self, resolve: TextIdk) -> Vec<TextIdk> {
        let mut result = JAPANESE_TRANSFORMS
            .iter()
            .flat_map(|transform| {
                transform
                    .rules
...
#[cfg(test)]
mod test {
    use crate::transform::JAPANESE_TRANSFORMS;
    ...
    fn thing() {
        ...
        let mut transform = TextTransform::new(text.to_owned());

        println!("{:#?}", transform.resolve());
