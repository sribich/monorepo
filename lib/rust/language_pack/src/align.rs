...
/// Represents a single alignment block between two vectors.
///
/// An alignment is stored in the following format, where the
/// range produced by a_pos & a_len maps onto b_pos & b_len:
///
///   [(a_pos, a_len), (b_pos, b_len)]
///
/// A range will _generally_ cleanly map 1:1 from source to
/// destination, but the range may occasionally have an
/// asymmetric mapping. How this is resolved is left up to
/// the caller.
pub struct Alignment(pub [(usize, usize); 2]);
...
/// Creates a list of alignments between `a` and `b`.
pub fn align_segments<TSegmentData, TSourceA, TSourceB>(
    a: &[Segment<TSegmentData, TSourceA>],
    b: &[Segment<TSegmentData, TSourceB>],
) -> Vec<Alignment>
where
    TSegmentData: IsSegment + Send + Sync + Debug,
    TSourceA: Send + Sync,
    TSourceB: Send + Sync,
{
    let aligner = SegmentAligner::new(a, b);
    ...
    aligner
        ...
        .map(|it| match it {
            ...
            | MatchKind::Empty {
                ...
                text_pos,
                text_length,
            } => Alignment([(timing_pos, timing_length), (text_pos, text_length)]),
            _ => {
                unreachable!()
            }
        })
        .filter(|it| it.0[0].1 != 0_usize && it.0[1].1 != 0_usize)
        .collect::<Vec<_>>()
}

struct SegmentAligner<'a, TSegmentData, TSourceA, TSourceB>
where
    TSegmentData: IsSegment + Send + Sync + Debug,
    TSourceA: Send + Sync,
    TSourceB: Send + Sync,
{
    a: &'a [Segment<TSegmentData, TSourceA>],
    b: &'a [Segment<TSegmentData, TSourceB>],
    matches: Vec<MatchKind>,
}

impl<'a, TSegmentData, TSourceA, TSourceB> SegmentAligner<'a, TSegmentData, TSourceA, TSourceB>
where
    TSegmentData: IsSegment + Send + Sync + Debug,
    TSourceA: Send + Sync,
    TSourceB: Send + Sync,
{
    ...
    fn align(mut self) -> Vec<MatchKind> {
        for _ in 0..100 {
    ...
    fn hirschberg_align(&mut self) {
        self.matches.par_iter_mut().for_each(|x| {
            ...
            } = &x
            {
                ...
                let matches = crate::hirschberg::Config::default().compute(timing, text);
